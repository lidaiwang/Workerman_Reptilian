...
class Rongcloud{
     ...
     * @param string $format
     */
    public function __construct($appkey, $seckey, $format = 'json'){
    	$this->appKey = $appkey;
    	$this->appSecret = $seckey;
        $this->format = $format;
    }
    ...
    public function getToken($userId, $name, $portraitUri) {
        try{
            ...
            return $this->_errArr($e->getMessage());
        }
    }
    ...
    public function _errArr($msg){
    	return array('success' => false, 'error' => $msg, 'errCode' => 304);
    }
    
    public function _successArr($data){
    	return array('success' => false, 'error' => '', 'data' => $data, 'errCode' => 200);
    }
}